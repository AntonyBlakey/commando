for (let columns_per_row = Math.max( ...bindings ); columns_per_row > 0; columns_per_row--) {
    ...
    for (let group in bindings) {
        ...
        for (let binding = 0; binding < bindings[group]; binding++) {
            ...
            key.style.setProperty("grid-row",row);
            key.style.setProperty("grid-column", 2 * column);
            const label = document.getElementById(`label-${group}-${binding}`);
            label.style.setProperty("grid-row", row);
            label.style.setProperty("grid-column", 2 * column + 1);
            if (row == start_row) {
        ...
        if (group_label) {
            group_label.style.setProperty("grid-row-start", start_row);
            group_label.style.setProperty("grid-row-end", row + 1); // non-exclusive
            group_label.style.setProperty("grid-column", 1);
        ...
        if (group_background) {
            group_background.style.setProperty("grid-row-start", start_row);
            group_background.style.setProperty("grid-row-end", row + 1); // non-exclusive
            group_background.style.setProperty("grid-column-start", 1);
            group_background.style.setProperty("grid-column-end", 2 * columns_per_row + 2);
            group_background.style.setProperty("margin-top", "0.2em");
        }
        row++;
    }
    if (document.getElementById("content").offsetWidth <= document.getElementById("body").offsetWidth)
        break;
}
