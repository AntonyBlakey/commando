...
pub type Command =  model::Command;
pub type Event =  model::Event;
...
impl Model {
    pub fn new(files: Vec<PathBuf>) -> Model {
        ...
        let definitions = model
            ...
            .map(|source|  model::ConfigFile::from_string(&source).unwrap())
            .flat_map(|config| config.definitions);

        for def in definitions {
            match def {
                 model::Definition::Root { commands, handlers } => {
...
impl Definition {
    fn new(
        from_guard: & model::Guard,
        from_keys: & model::KeyMap,
        from_groups: &Vec< model::GroupDefinition>,
    ) -> Definition {
...
impl Binding {
    fn new(from: & model::Binding, group_label: Option<DisplayLabel>) -> Binding {
        match from {
             model::Binding::Command {
                label,
                command,
                r#loop: false,
                select_window: _,
             ...
             model::Binding::Mode { label, mode } => Binding::Mode {
                group_label,
                label: label.clone(),
                mode: mode.clone(),
            },
